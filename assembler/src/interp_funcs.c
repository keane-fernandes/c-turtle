...
void init_turtle(turtle* turt)
{
    ...
    turt->rgb.red = UINT8_MAX;
    turt->rgb.green = UINT8_MAX;
    turt->rgb.blue = UINT8_MAX;

    turt->current_bitwise = UNDEFINED;
    turt->current_reg = UNDEFINED;
    turt->dst_reg = UNDEFINED;
    turt->op1_reg = UNDEFINED;
    turt->op2_reg = UNDEFINED;
...
void move_turtle(program* prog, turtle* turt, float new_x, float new_y)
{
    ...
    Neill_SDL_SetDrawColour(&(prog->sw), turt->rgb.red, \
        turt->rgb.green, turt->rgb.blue);
